...
namespace {
    ...
    static inline double _timedarray_2(const double t)
    {
        ...
        if(i >= 20000)
            i = 20000-1;
        return _namespace_timedarray_2_values[i];
    }
...
void _run_neurongroup_stateupdater_codeobject_2()
{
    ...
    ///// CONSTANTS ///////////
    const double C = 2.790940326009133e-11;
const double El = - 0.052442050364073636;
const double G = 8.34449007948743e-09;
const size_t _numI_0 = 1;
const size_t _numI_1 = 1;
const int32_t N = 1;
const double Th_inf = - 0.045524579716594425;
const size_t _numTh_s = 1;
const size_t _numTh_v = 1;
const size_t _numV = 1;
const double a_v = 89.98285185059846;
const double b_s = 10.965293075296792;
const double b_v = 51.01649693305835;
...
const size_t _numt = 1;
    ...
    double*   _ptr_array_neurongroup_lastspike_2 = _array_neurongroup_lastspike_2;
    ...
    const int32_t _lio_1 = _timestep(0.003535, dt);
    ...
    const double _lio_8 = 0.0 - (1.0f*((((- G) * Th_inf) * b_v) - (a_v * _timedarray_2(t)))/(G * b_v));
    ...
    const double _lio_14 = 1.0f*((C * a_v) * (((- El) * G) - _timedarray_2(t)))/(G * ((C * b_v) - G));
    ...
    const double _lio_22 = 1.0f*((((- G) * Th_inf) * b_v) - (a_v * _timedarray_2(t)))/(G * b_v);
    ...
    const double _lio_31 = 0.0 - (1.0f*(((- El) * G) - _timedarray_2(t))/G);
    const double _lio_32 = 1.0f*1.0/((C * k_0) - G);
    const double _lio_33 = 1.0f*1.0/((C * k_1) - G);
    const double _lio_34 = 1.0f*(((- El) * G) - _timedarray_2(t))/G;
    ...
    for(int _idx=0; _idx<_N; _idx++)
    {
        ...
        const double lastspike = _ptr_array_neurongroup_lastspike_2[_idx];
        char not_refractory = _ptr_array_neurongroup_not_refractory_2[_idx];
        ...
        _ptr_array_neurongroup_not_refractory_2[_idx] = not_refractory;
