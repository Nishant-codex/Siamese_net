...
namespace {
    ...
    static inline double _timedarray(const double t)
    {
        ...
        if(i >= 20000)
            i = 20000-1;
        return _namespace_timedarray_values[i];
    }
...
void _run_neurongroup_stateupdater_codeobject()
{
    ...
    ///// CONSTANTS ///////////
    const double C = 2.8694867473874465e-11;
const double El = - 0.05532817877943799;
const double G = 8.364369857853948e-09;
const size_t _numI_0 = 1;
const size_t _numI_1 = 1;
const int32_t N = 1;
const double Th_inf = - 0.04989861948521157;
const size_t _numTh_s = 1;
const size_t _numTh_v = 1;
const size_t _numV = 1;
const double a_v = 2.8444641774354107;
const double b_s = 9.476572718746066;
const double b_v = 83.57815274094777;
...
const size_t _numt = 1;
    ...
    const int32_t _lio_1 = _timestep(0.004243, dt);
    ...
    const double _lio_8 = 0.0 - (1.0f*((((- G) * Th_inf) * b_v) - (a_v * _timedarray(t)))/(G * b_v));
    ...
    const double _lio_14 = 1.0f*((C * a_v) * (((- El) * G) - _timedarray(t)))/(G * ((C * b_v) - G));
    ...
    const double _lio_22 = 1.0f*((((- G) * Th_inf) * b_v) - (a_v * _timedarray(t)))/(G * b_v);
    ...
    const double _lio_31 = 0.0 - (1.0f*(((- El) * G) - _timedarray(t))/G);
    const double _lio_32 = 1.0f*1.0/((C * k_0) - G);
    const double _lio_33 = 1.0f*1.0/((C * k_1) - G);
    const double _lio_34 = 1.0f*(((- El) * G) - _timedarray(t))/G;
