...
const size_t _numt = 1;
    ...
    for(size_t _idx=0; _idx<N; _idx++)
    {
        ...
        const double Th_s = _ptr_array_neurongroup_Th_s_2[_idx];
        const double Th_v = _ptr_array_neurongroup_Th_v_2[_idx];
        const double V = _ptr_array_neurongroup_V_2[_idx];
        const char not_refractory = _ptr_array_neurongroup_not_refractory_2[_idx];
        char _cond;
        if(!not_refractory)
            _cond = (V > (Th_v + Th_s)) && false;
        else 
            _cond = (V > (Th_v + Th_s)) && true;

        if(_cond) {
            _ptr_array_neurongroup__spikespace_2[_count++] = _idx;
            _ptr_array_neurongroup_not_refractory_2[_idx] = false;
            _ptr_array_neurongroup_lastspike_2[_idx] = _ptr_array_defaultclock_t[0];
        }
    }
    _ptr_array_neurongroup__spikespace_2[N] = _count;
